...
public class MainForm : Form
{
    ...
    private List<ShippingLabelManager.Models.ShippingSite> _shippingSites = new();
    ...
    // Report data
    private List<ShippingLabelManager.Models.ReturnLabelReport>   _returnReportData   = new();
    private List<ShippingLabelManager.Models.ShippingLabelReport> _shippingReportData = new();
    private ShippingLabelManager.Models.ReportSummary?            _reportSummary;
    private List<ShippingLabelManager.Models.SiteBreakdown>       _siteBreakdown      = new();
    ...
    private void BuildReturnTab()
    {
        ...
        // Config group
        var grpConfig = new GroupBox { Text = "Label Configuration", Dock = DockStyle.Top, Height = 110, Padding = new Padding(10) };
        ...
        grpConfig.Controls.AddRange(new Control[] { lblSite, _cboSite, lblSiteHint, lblQty, _numQty });
        ...
        LayoutButtons(pnlBtns, new Control[] { _btnExportCSV, _btnOpenWord });
    ...
    private void BuildShippingTab()
    {
        ...
        _dgvShipping = MakeGrid(false);   // not fully read-only — Boxes column is editable
    ...
    private void OnExportCSV(object? s, EventArgs e)
    {
        ...
        using var dlg = new SaveFileDialog
        {
            ...
            FileName         = "labels_" + site.LIBCODE + "_" + from.ToString("D9") + "_qty" + qty + ".csv",
            InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop)
        };
        if (dlg.ShowDialog() != DialogResult.OK) return;

        // Build and write CSV — sequence NOT advanced yet
        try { File.WriteAllText(dlg.FileName, BarcodeService.ToCsv(BarcodeService.BuildBatch(site, from, qty))); }
        catch (Exception ex) { MessageBox.Show("Could not write CSV:" + Environment.NewLine + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
        ...
        // Confirmed — advance sequence and log history
        var actualFrom = Program.DB.AdvanceSequence(site.Id, qty, out var newNext);
        site.CurrentSequence = newNext; site.LastPrintedSerial = actualFrom + qty - 1; site.LastPrintedDate = DateTime.Now;
        Program.DB.InsertHistory(new PrintHistoryEntry { SiteId = site.Id, LIBCODE = site.LIBCODE, SiteName = site.SiteName, SerialFrom = actualFrom, SerialTo = actualFrom + qty - 1, LabelCount = qty, RunType = "CSV", LabelSize = "", PrintedBy = Environment.UserName });
        UpdateStats();
        if (_tabs.SelectedTab == _tabHistory) LoadHistory();
        RefreshGeneratePreview();
    }

    // ── Export & open in Word — launches persistent print session ─
    private void OnOpenWord(object? s, EventArgs e)
    {
        ...
        // Build CSV using current sequence — NOT advanced yet
        var csvPath = Path.Combine(AppSettings.TempFolder, "labels_" + site.LIBCODE + "_current.csv");
        try { File.WriteAllText(csvPath, BarcodeService.ToCsv(BarcodeService.BuildBatch(site, from, qty))); }
        catch (Exception ex) { MessageBox.Show("Could not write temp CSV:" + Environment.NewLine + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }

        // Open the session — each Merge & Print logs an ATTEMPT immediately via the callback.
        // Sequence only advances when user clicks Confirm.
        using var session = new PrintSessionForm(site, from, qty, csvPath, AppSettings.WordTemplates,
            (templateName, runType) =>
            {
        ...
        {
            // Confirmed — advance sequence and log final WORD entry
            var actualFrom = Program.DB.AdvanceSequence(site.Id, qty, out var newNext);
            site.CurrentSequence = newNext; site.LastPrintedSerial = actualFrom + qty - 1; site.LastPrintedDate = DateTime.Now;
            Program.DB.InsertHistory(new PrintHistoryEntry
            {
        ...
        {
            // Cancelled — log cancellation if at least one attempt was made
    ...
    private void OnShipExportCSV(object? s, EventArgs e)
    {
        ...
        {
            MessageBox.Show(
                "No sites selected for printing." + Environment.NewLine + Environment.NewLine +
                "Either:" + Environment.NewLine +
                "  • Check the checkbox next to one or more sites" + Environment.NewLine +
                "  • Or select a single site row and enter a Boxes quantity",
        ...
        {
            MessageBox.Show(
                "No active shipping site is configured." + Environment.NewLine + Environment.NewLine +
                "Go to Manage Sites → Shipping Sites to add and activate a shipping site.",
                "No active shipping site", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        using var dlg = new SaveFileDialog
        {
            ...
            Filter           = "CSV files (*.csv)|*.csv",
            FileName         = "shipping_labels_" + DateTime.Now.ToString("yyyyMMdd") + ".csv",
            InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop)
        };
        if (dlg.ShowDialog() != DialogResult.OK) return;

        int firstSerial = Program.DB.AdvanceShippingSequence(totalLabels, out _);
        File.WriteAllText(dlg.FileName, BarcodeService.ToShippingCsv(entries, activeSite, firstSerial));
        SaveShippingQuantities(entries);
        LogShippingHistory(entries, "SHIP", activeSite, firstSerial, totalLabels);
        UpdateStats();

        MessageBox.Show(
            "CSV exported: " + entries.Count + " site" + (entries.Count > 1 ? "s" : "") + ",  " +
            totalLabels.ToString("N0") + " total label" + (totalLabels > 1 ? "s" : "") +
            Environment.NewLine + Environment.NewLine + "File: " + dlg.FileName,
    ...
    private void OnShipOpenWord(object? s, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(AppSettings.TempFolder))
        {
            MessageBox.Show("Please configure a Temp CSV folder in File > Settings.", "Settings required", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        if (string.IsNullOrEmpty(AppSettings.ShippingTemplatePath))
        {
            MessageBox.Show("Please configure a Shipping label template in File > Settings.", "Settings required", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        var csvPath = Path.Combine(AppSettings.TempFolder, "shipping_labels_current.csv");
        File.WriteAllText(csvPath, BarcodeService.ToShippingCsv(entries, activeSite, firstSerial));
        SaveShippingQuantities(entries);
        LogShippingHistory(entries, "SHIP", activeSite, firstSerial, totalLabels);
        UpdateStats();
        ...
        try
        {
            WordMergeService.OpenInWord(csvPath, AppSettings.ShippingTemplatePath);
            MessageBox.Show(
                "Word has been opened with the shipping labels mail merge." + Environment.NewLine + Environment.NewLine +
                "Review and print from Word when ready.",
                "Opened in Word", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        catch (Exception ex)
        {
            MessageBox.Show("Could not open Word:" + Environment.NewLine + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    private void LogShippingHistory(List<(Site site, int boxes)> entries, string runType,
        ShippingLabelManager.Models.ShippingSite shippingSite, int firstSerial, int totalBoxes)
    {
        int serialOffset = 0;
        foreach (var (site, boxes) in entries)
        {
    ...
    private void OpenShippingSiteForm(ShippingLabelManager.Models.ShippingSite? site)
    {
    ...
    private ShippingLabelManager.Models.ShippingSite? GetSelectedShippingSiteFromGrid() =>
    ...
    {
        using var dlg = new SaveFileDialog { Title = "Save import template", Filter = "CSV files (*.csv)|*.csv", FileName = "sites_import_template.csv", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
    ...
    private void ExportSitesToCsv()
    {
        ...
        using var dlg = new SaveFileDialog { Title = "Export all sites", Filter = "CSV files (*.csv)|*.csv", FileName = "sites_export_" + DateTime.Now.ToString("yyyyMMdd") + ".csv", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
    ...
    private static string[] ReturnRowMapper(ShippingLabelManager.Models.ReturnLabelReport r) =>
    ...
    private static string[] ShipRowMapper(ShippingLabelManager.Models.ShippingLabelReport r) =>
        new[] { r.PrintedAt.ToString("yyyy-MM-dd HH:mm"), r.LIBCODE, r.SiteName,
    ...
    private void OnExportReturnCsv(object? s, EventArgs e)
    {
        ...
        using var dlg = new SaveFileDialog { Filter = "CSV files (*.csv)|*.csv", FileName = "return_labels_" + DateTime.Now.ToString("yyyyMMdd") + ".csv", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
        ShippingLabelManager.Services.ReportExportService.ExportToCsv(_returnReportData, ReturnCsvHeaders, ReturnRowMapper, dlg.FileName);
    ...
    {
        if (_returnReportData.Count == 0) { MessageBox.Show("No data to export.", "Empty", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        using var dlg = new SaveFileDialog { Filter = "Excel files (*.xlsx)|*.xlsx", FileName = "return_labels_" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
        ShippingLabelManager.Services.ReportExportService.ExportToExcel(_returnReportData, ReturnCsvHeaders, ReturnRowMapper, dlg.FileName, "Return Labels");
        MessageBox.Show("Exported: " + dlg.FileName, "Export complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    ...
    {
        if (_returnReportData.Count == 0) { MessageBox.Show("No data to export.", "Empty", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        using var dlg = new SaveFileDialog { Filter = "PDF files (*.pdf)|*.pdf", FileName = "return_labels_" + DateTime.Now.ToString("yyyyMMdd") + ".pdf", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
        ...
        ShippingLabelManager.Services.ReportExportService.ExportToPdf(_returnReportData, ReturnCsvHeaders, ReturnRowMapper, dlg.FileName, "Return Labels Report", subtitle);
        MessageBox.Show("Exported: " + dlg.FileName, "Export complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    ...
    private void OnExportShipCsv(object? s, EventArgs e)
    {
        ...
        using var dlg = new SaveFileDialog { Filter = "CSV files (*.csv)|*.csv", FileName = "shipping_report_" + DateTime.Now.ToString("yyyyMMdd") + ".csv", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
        ShippingLabelManager.Services.ReportExportService.ExportToCsv(_shippingReportData, ShipCsvHeaders, ShipRowMapper, dlg.FileName);
        MessageBox.Show("Exported: " + dlg.FileName, "Export complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    ...
    {
        if (_shippingReportData.Count == 0) { MessageBox.Show("No data to export.", "Empty", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        using var dlg = new SaveFileDialog { Filter = "Excel files (*.xlsx)|*.xlsx", FileName = "shipping_report_" + DateTime.Now.ToString("yyyyMMdd") + ".xlsx", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
        ShippingLabelManager.Services.ReportExportService.ExportToExcel(_shippingReportData, ShipCsvHeaders, ShipRowMapper, dlg.FileName, "Shipping Labels");
        MessageBox.Show("Exported: " + dlg.FileName, "Export complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
    ...
    {
        if (_shippingReportData.Count == 0) { MessageBox.Show("No data to export.", "Empty", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        using var dlg = new SaveFileDialog { Filter = "PDF files (*.pdf)|*.pdf", FileName = "shipping_report_" + DateTime.Now.ToString("yyyyMMdd") + ".pdf", InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.Desktop) };
        if (dlg.ShowDialog() != DialogResult.OK) return;
        var subtitle = _dtpFrom.Value.ToString("yyyy-MM-dd") + " to " + _dtpTo.Value.ToString("yyyy-MM-dd") +
                       (_cboReportSite.SelectedIndex > 0 ? "  |  " + _cboReportSite.SelectedItem : "  |  All sites");
        ShippingLabelManager.Services.ReportExportService.ExportToPdf(_shippingReportData, ShipCsvHeaders, ShipRowMapper, dlg.FileName, "Shipping Labels Report", subtitle);
        MessageBox.Show("Exported: " + dlg.FileName, "Export complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
