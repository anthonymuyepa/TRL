...
public static class ReportExportService
{
    ...
    private static void ExportRowsToCsv(string[] headers, List<string[]> rows, string filePath)
    {
        var sb = new StringBuilder();
        sb.AppendLine(string.Join(",", headers.Select(h => "\"" + h + "\"")));
        ...
        {
            sb.AppendLine(string.Join(",",
                row.Select(c => "\"" + (c ?? "").Replace("\"", "\"\"") + "\"")));
        }
        File.WriteAllText(filePath, sb.ToString());
    }

    private static void ExportRowsToExcel(string[] headers, List<string[]> rows, string filePath, string sheetName, string reportTitle, string subtitle)
    {
        using var document = SpreadsheetDocument.Create(filePath, SpreadsheetDocumentType.Workbook);
        ...
        titleRow.AppendChild(MakeCell(CellRef(0, 1), reportTitle, styleIndex: 1));
        sheetData.AppendChild(titleRow);

        var subtitleRow = new Row { RowIndex = 2 };
        subtitleRow.AppendChild(MakeCell(CellRef(0, 2), subtitle, styleIndex: 0));
        sheetData.AppendChild(subtitleRow);

        // Header row
        var headerRow = new Row { RowIndex = 4 };
        for (int i = 0; i < headers.Length; i++)
            headerRow.AppendChild(MakeCell(CellRef(i, 4), headers[i], styleIndex: 1));
        sheetData.AppendChild(headerRow);
        ...
        {
            var style = (uint)(rowIdx % 2 == 0 ? 2 : 0);
            var excelRow   = new Row { RowIndex = rowIdx };
            for (int i = 0; i < row.Length; i++)
                excelRow.AppendChild(MakeCell(CellRef(i, (int)rowIdx), row[i] ?? "", styleIndex: style));
    ...
    private static void ExportRowsToPdf(string[] headers, List<string[]> rows, string filePath, string reportTitle, string subtitle)
    {
        QuestPDF.Settings.License = LicenseType.Community;
    ...
    // ── RETURN LABELS DETAIL EXPORT ───────────────────────────

    public static void ExportReturnLabelsDetail(
        List<TRL.Models.ReturnLabelReport> runs,
        List<TRL.Models.Site> sites,
        string filePath,
        ExportFormat format,
        string reportTitle,
        string subtitle)
    {
        // Build expanded rows — one per label
        var rows = new List<string[]>();
        
        foreach (var run in runs)
        {
            var site = sites.FirstOrDefault(
                s => s.LIBCODE == run.LIBCODE);
            if (site == null) continue;

            for (int serial = run.SerialFrom; 
                 serial <= run.SerialTo; 
                 serial++)
            {
                var label = BarcodeService.BuildLabelRow(
                    site, serial);
                rows.Add(new[]
                {
                    run.PrintedAt.ToString("yyyy-MM-dd HH:mm"),
                    run.LIBCODE,
                    run.SiteName,
                    serial.ToString("D9"),
                    label.TrackingCore,
                    label.MOD10.ToString(),
                    label.GS1_128_Raw,
                    label.HR_Text,
                    run.RunType,
                    run.TemplateName ?? "",
        ...
        var headers = new[]
        {
            "Date & time", "LIBCODE", "Site",
            "Serial", "TrackingCore", "MOD10",
            "GS1_128_Raw", "HR_Text",
            "Type", "Template", "Size"
        };
        ...
        switch (format)
        {
            case ExportFormat.Csv:
                ExportRowsToCsv(headers, rows, filePath);
                break;
            case ExportFormat.Excel:
                ExportRowsToExcel(headers, rows, filePath,
                    "Return Labels Detail", reportTitle, subtitle);
                break;
            case ExportFormat.Pdf:
                ExportRowsToPdf(headers, rows, filePath,
