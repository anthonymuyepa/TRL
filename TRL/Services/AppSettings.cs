...
public static class AppSettings
{
    ...
    public static int CalibrationCount         { get; set; } = 20;
    public static int BatchSize                { get; set; } = 1000;
    public static int WordMergeTimeoutMinutes  { get; set; } = 30;
    public static int SerialDisplayDigits      { get; set; } = 6;  // NEW: 6 or 9 digits for serial display
    ...
    public static void Load()
    {
        try
        {
            ...
            if (root.TryGetProperty("CalibrationCount",     out var cc))  CalibrationCount      = cc.GetInt32();
            if (root.TryGetProperty("BatchSize",            out var bs))  BatchSize             = bs.GetInt32();
            if (root.TryGetProperty("WordMergeTimeoutMinutes", out var tm)) WordMergeTimeoutMinutes = tm.GetInt32();
            if (root.TryGetProperty("SerialDisplayDigits",  out var sd))  SerialDisplayDigits   = sd.GetInt32();

            // Validate ranges
            if (CalibrationCount < 0) CalibrationCount = 0;
            if (CalibrationCount > 100) CalibrationCount = 100;
            if (BatchSize < 500) BatchSize = 500;
            if (BatchSize > 5000) BatchSize = 5000;
            if (WordMergeTimeoutMinutes < 10) WordMergeTimeoutMinutes = 10;
            if (WordMergeTimeoutMinutes > 120) WordMergeTimeoutMinutes = 120;
            if (SerialDisplayDigits != 6 && SerialDisplayDigits != 9) SerialDisplayDigits = 6;  // Only allow 6 or 9
