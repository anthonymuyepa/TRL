...
public class PrintSessionForm : Form
{
    ...
    private readonly string _outputDocxPath;  // NEW
    ...
    public PrintSessionForm(Site site, int fromSerial, int qty, string csvPath, string outputDocxPath,
                            List<WordTemplate> templates, Action<string, string>? onPrintAttempt = null)
    {
        ...
        _qty = qty;
        _csvPath = csvPath;
        _outputDocxPath = outputDocxPath;  // NEW
    ...
    private void InitializeComponent()
    {
        ...
        {
            Text      = "[TRL] Ready — select a template and click Create Word Doc",
            Dock      = DockStyle.Fill,
...
private void OnOpenInWord(object? s, EventArgs e)
{
    ...
    _lstLog.Items.Add("[TRL]  " + timestamp + "  " + template.Name + "  →  Create Word Doc initiated");
    _lstLog.TopIndex = _lstLog.Items.Count - 1;
    _lstLog.Refresh();

    // Disable Confirm button during processing
    _btnConfirm.Enabled = false;
    _btnConfirm.BackColor = Color.FromArgb(180, 180, 180);
    ...
    var ts = timestamp;
    var siteLibcode = _site.LIBCODE ?? "SITE";
    var fromSerial = _fromSerial;
    ...
    Task.Run(() =>
    {
        ...
        try
        {
            ...
            var safeLibcode = siteLibcode;
            foreach (var c in Path.GetInvalidFileNameChars())
                safeLibcode = safeLibcode.Replace(c, '_');

            safeLibcode = safeLibcode.Trim().Replace(" ", "_");

            var startSerialText = fromSerial < 10000
                ? fromSerial.ToString("D4")
                : fromSerial.ToString();

            outputDocxPath = Path.Combine(
                outputFolder,
                $"{safeLibcode}_{startSerialText}.docx");

            if (File.Exists(outputDocxPath))
            {
                try { File.Delete(outputDocxPath); }
                catch
                {
                    // If locked, SaveMergedDocument will report a clearer error.
                }
            }

            // FIXED: Added metadata parameters
            WordMergeService.SaveMergedDocument(
                csvPath,
                templatePath,
                outputDocxPath,
                siteName: _site.SiteName,
                startSerial: _fromSerial,
                endSerial: _toSerial
            );
            ...
            var fileInfo = new FileInfo(outputDocxPath);
            if (fileInfo.Length == 0)
                throw new Exception("Word document was created but is 0 bytes.");

            success = true;

            resultMsg =
                "[TRL]  " + ts + "  " + templateName +
                "  →  Document saved: " + Path.GetFileName(outputDocxPath);
        }
        catch (Exception ex)
        {
            resultMsg =
                "[TRL]  " + ts + "  ERROR  " + templateName +
                "  →  " + ex.Message;
        }
        ...
        Invoke(() =>
        {
            ...
            if (idx >= 0)
                _lstLog.Items[idx] = resultMsg;

            _lstLog.TopIndex = _lstLog.Items.Count - 1;
            _lstLog.Refresh();

            // Re-enable Confirm button after processing completes
            _btnConfirm.Enabled = true;
            _btnConfirm.BackColor = Color.FromArgb(0, 150, 50);
            
            _btnOpen.Enabled = true;
            _btnOpen.Text = "Create Word Doc";
            ...
            if (success)
            {
                ...
                {
                    MessageBox.Show(
                        "The document was created, but could not be opened automatically:" +
